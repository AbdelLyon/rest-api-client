...
export type RelationDefinitionType = "create" | "update" | "attach" | "detach" | "sync" | "toggle";
...
export type ValidCreateNestedRelation<T> = (CreateRelationOperation<T> & {
    relations?: Record<string, ValidCreateNestedRelation<any>>;
}) | AttachRelationDefinition;
export type ValidUpdateNestedRelation<T> = (CreateRelationOperation<T> & {
    relations?: Record<string, ValidCreateNestedRelation<any>>;
...
export type RelationDefinition<T = unknown, TInCreateContext extends boolean = false> = TInCreateContext extends true ? ValidCreateNestedRelation<T> : ValidUpdateNestedRelation<T>;
...
export type CreateEntityAttributes<T, TRelationKeys extends keyof T = never> = {
    [K in keyof T]: K extends TRelationKeys ? IsRelationOperation<T[K]> extends true ? ValidCreateRelationOnly<T[K]> : T[K] : T[K];
};
